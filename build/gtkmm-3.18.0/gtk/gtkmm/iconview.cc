...
{
  gtk_icon_view_set_cursor(gobj(), const_cast<GtkTreePath*>((path).gobj()), 0, static_cast<int>(start_editing));
}
...
{
  GdkRectangle g_rect = GdkRectangle();
  bool retvalue = gtk_icon_view_get_cell_rect(const_cast<GtkIconView*>(gobj()), const_cast<GtkTreePath*>((path).gobj()), 0, &g_rect);
rect = Glib::wrap(&(g_rect));
  return retvalue;
}
