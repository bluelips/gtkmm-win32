...
{
  gtk_menu_shell_bind_model(gobj(), Glib::unwrap(model), action_namespace.empty() ? 0 : action_namespace.c_str(), static_cast<int>(with_separators));
}

void MenuShell::bind_model(const Glib::RefPtr<Gio::MenuModel>& model, bool with_separators)
{
  gtk_menu_shell_bind_model(gobj(), Glib::unwrap(model), 0, static_cast<int>(with_separators));
}
